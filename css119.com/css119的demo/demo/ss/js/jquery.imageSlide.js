...
; (function($) {
    ...
    ImageSlide.prototype = {
        ...
        move: function() {
            ...
            //防止连续点击多次，动画执行多次
            if (!self.dbclicks) {
                self.prevIndex = $(".prevIndexLi").index();
                //向左运动
                if (arguments[0] == 0) {
                    //不循环的时候
                    if (!self.isLoop) {
        ...
        moving: function(obj, space, dir, callback) {
			...
			//alert(self.li.not(self.li.eq(self.prevIndex)))
            clearInterval(self.timer2);
			if (self.prevIndex < self.index) {
                        self.li.not(self.li.eq(self.prevIndex)).stop(true,true).css(self.direction, self.step)
                    } else {
                       self.li.not(self.li.eq(self.prevIndex)).stop(true,true).css(self.direction, -self.step)
                    }
            self.timer2 = setInterval(function() {
                ...
                obj.stop(true,true).css(self.direction, offset + iSpeed);
                var timeSpace = parseInt(obj.css(self.direction));
                if (dir == 0) {
