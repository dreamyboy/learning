...
@example: 
	$(id).imageSlide({
        ...
        effect: 1,
        //滑动效果（只对滑动效果有作用） 0：匀速滑动；1：缓冲滑动（越来越慢）；2：反弹性滑动
        slideType: 0,
        //效果时间
        speed: 500,
        //效果间隔时间
...
; (function($) {
    ...
    ImageSlide.prototype = {
        ...
        init: function() {
            var self = this;
            self.isLoadJSON && self.isLoadJSONFun();
            self.config();
            self.isAuto && self.autoPlay();
            self.hotKey && self.hotKeyFun();
            self.bind();
        },
        /*配置*/
        config: function() {
            ...
            case 0:
                ...
                //避免渐变的间隔时候出现闪白的情况
                self.li.css("position", "absolute").hide().eq(self.index).show();
                break;
                //横向滑动
            case 1:
                self.direction = "left";
                ...
                //循环时
                if (self.isLoop) {
                    self.ul.width(self.step * (self.len + 1));
                }
                //不循环时
                else {
                    self.ul.width(self.step * self.len);
                }
                break;
                //垂直滑动
            case 2:
                ...
                if (self.isLoop) {
                    self.ul.height(self.step * (self.len + 1));
                } else {
                    self.ul.height(self.step * self.len);
                }
                break;
            }
            //滑动效果共同的部分
            if (self.effect == 1 || self.effect == 2) {
                ...
                if (self.isLoop) {
                    ...
                    self.lazyLoad && !self.loadImg(self.index) && self.loadImg(0,
                    function() {
                        self.li.eq(0).clone().appendTo(self.ul);
                    });
                    //否则
                    ! self.lazyLoad && self.li.eq(0).clone().appendTo(self.ul);
                }
                //不循环时
                else {
                    self.lazyLoad && !self.loadImg(self.index);
                }
                self.ul.stop(true, true).css(self.direction, -self.step * self.index);
            }
        },
        /*动画执行完后的回调函数*/
                ...
                case 2:
                    ...
                    //向右运动
                        ...
                        if (!self.isLoop && self.index > 0) {
                            ...
                            self.index--;
                            if (self.index < 0) {
                                self.ul.css(self.direction, -self.step * self.len);
                                self.index = self.len - 1;
                            }
                        }
                    }
                    self.moveObj[self.direction] = -self.step * self.index + "px";
                    //滑动效果（只对滑动效果有作用） 0：匀速滑动；1：缓冲滑动（越来越慢）；2：反弹性滑动
                    switch (self.slideType) {
                    case 0:
                    default:
                        self.ul.stop(true, true).animate(self.moveObj, self.speed,
                        function() {
                            self.moveCallback(self.index)
                        });
                        break;
                    case 1:
                        self.moving(self.ul, -self.step * self.index,
                        function() {
                            self.moveCallback(self.index)
                        });
                        break;
                    case 2:
                        self.reMoving(self.ul, -self.step * self.index,
                        function() {
                            self.moveCallback(self.index)
                        });
                        break;
        ...
        /*定义缓冲滑动函数*/
        moving: function(obj, space, callback) {
            var self = this;
            //此效果下，speed最小为200
            ...
            self.timer2 = setInterval(function() {
                ...
                if (offset == space) {
                    clearInterval(self.timer2);
                    obj.css(self.direction, offset + "px");
                    callback && typeof callback == "function" && callback();
                }
                obj.css(self.direction, offset + iSpeed + "px");
            },
            30);
        },
        /*定义反弹性滑动函数*/
        reMoving: function(obj, space, callback) {
            var self = this;
            //此效果下，speed最小为200
            self.speed = self.speed > 200 ? self.speed: 200;
            var space = parseInt(space),
            ...
            self.direction == "left" && (left = obj[0].offsetLeft);
            self.direction == "top" && (left = obj[0].offsetTop);
            self.timer3 = setInterval(function() {
                self.direction == "left" && (offset = obj[0].offsetLeft);
                self.direction == "top" && (offset = obj[0].offsetTop);
                iSpeed += (space - offset) / (self.speed / 100);
                iSpeed *= 0.7;
                left += iSpeed;
                if (Math.abs(iSpeed) < 1 && Math.abs(left - space) < 1) {
                    clearInterval(self.timer3);
                    obj.css(self.direction, space + "px");
                    callback && typeof callback == "function" && callback();
                    left < 0 && (left = 0);
                } else {
                    left += iSpeed;
                    obj.css(self.direction, left + "px");
                };
            },
            30);
        },
